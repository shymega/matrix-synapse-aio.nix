...
let
  ...
  wcfg = config.services.matrix-synapse.workers;
...
in
{
  ...
  options.services.matrix-synapse.workers =
    let
    ...
    in
    {
      mainReplicationHost = mkOption {
        ...
        default =
          let
            inherit (matrix-lib.connectionInfo.mainReplicationListener) host;
          in
  ...
  config = {
    ...
    services.matrix-synapse.settings = {
      ...
      instance_map = lib.mkIf (cfg.workers.instances != { }) ({
        main =
          if wcfg.mainReplicationPath != null then {
    ...
    services.matrix-synapse.workers.instances =
      let
        ...
        scanl = f: x1: list:
          let
            ...
            x1' = f x1 x2;
          in
          if list == [ ] then [ ] else [ x1' ] ++ (scanl f x1' (lib.tail list));
    ...
    systemd.services =
      ...
      in
      builtins.listToAttrs (lib.flip map workerList (worker: {
        ...
        value = {
          ...
          serviceConfig = {
            ...
            ExecStart =
              ...
              in
              "${wrapped}/bin/synapse_worker ${flags}";
          };
        };
      }));
  };
